...
- [dificultad](incidencia:incidencia) [st](estaciones:estaciones) [fal](codigoEstacion)
...
- quien [cerro](abrir) la [123456789](codigoIncidencia)
- quien [cerro](abrir) la [444444444](codigoIncidencia)
- quien ha [cerrado](abrir) la [123456](codigoIncidencia)
...
## story_operacion_check
* intent_incidencias_operacion{"incidencia": "incidencias", "operacion": "operación", "codigoOperacion": "1234"}
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if codigoOperacion != None:
            ...
            if (
                ...
                dispatcher.utter_message("Código de operación no válido")
                return []
            elif (
                int(codigoOperacion) < 7000000 or int(codigoOperacion) > 8999999
            ) and orden != None:
                dispatcher.utter_message("Código de orden no válido")
                return []
        else:
            if operacion != None:
                dispatcher.utter_message("Introduzca un numero de " + operacion)
            elif orden != None:
                dispatcher.utter_message("Introduzca un numero de " + orden)
            return []
        ...
        if tipoIncidencia == "obs":
            if int(codigoOperacion) > 0 and int(codigoOperacion) <= 9999:
                ...
                if str(rows[0]) != '(0,)':
                    ...
                    row = "".join(row)
                    rowOBS = row.replace(",", "").replace("(", "").replace(")", "")
                    dispatcher.utter_message("<b>Existen " + rowOBS + " OBS con el código de orden " + codigoOperacion + "</b>")
                    #obs = '1' #Borrar si se quiere listar
        ...
        if tipoIncidencia == 'demat':
            ...
            if str(rows[0]) != '(0,)':
                ...
                dispatcher.utter_message("<b>Existen " + rowDEMAT + " DEMAT con el código de orden " + codigoOperacion + "</b>")
        ...
        if tipoIncidencia == None:
            ...
            if tipoIncidencia == "hnc":
                tabla = "lm_fil_hnc"
                cur.execute(
                    "SELECT partnumber, texto_material_pn_hnc, orden, operacion, descripción_larga_defecto, texto_largo_descriptivo from lm_fil_hnc where operacion ='"
                    + codigoOperacion
                    + "';"
                )
                rows = cur.fetchall()
                if rows != []:
                    ...
                    for row in rows:
                        ...
                        dispatcher.utter_message(incidenciaDetallada)
                else:
                    incidenciaDetallada = ""

            elif tipoIncidencia == "obs":
                tabla = "lm_fil_observaciones"
                cur.execute(
                    "SELECT orden, operacion, observacion, descripcion_observacion from lm_fil_observaciones where operacion ='"
                    + codigoOperacion
                    + "';"
                )
                rows = cur.fetchall()
            ...
            if tipoIncidencia != None and tabla != "":
                cur.execute(
                    "SELECT count(*) from "
                    + tabla
                    + " where operacion ='"
        ...
        if int(rowIncidencia) < 1:
            if tipoIncidencia == "demat" and orden == None:
